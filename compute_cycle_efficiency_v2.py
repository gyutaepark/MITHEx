...
def compute_cycle_efficiency(loop_res, dP_core_primary=350e3):
    ...
    # Get Intermediate Loop Values
    intermediate_mdot = loop_res["Intermediate Mass Flow Rate (kg/s)"]
    intermediate_hot = Q_(loop_res["Intermediate Hot Temperature (C)"],
                          'degC').m_as('K')
    intermediate_cold = Q_(loop_res["Intermediate Cold Temperature (C)"],
                           'degC').m_as('K')
    intermediate_P = Q_(loop_res["Intermediate Pressure (kPa)"],
                        'kPa').m_as('Pa')
    ...
    # Get Secondary Loop Values
    secondary_fluid = loop_res["Secondary Fluid"]
    secondary_hot = Q_(loop_res["Secondary Hot Temperature (C)"],
                       'degC').m_as('K')
    secondary_cold = Q_(loop_res["Secondary Cold Temperature (C)"],
                        'degC').m_as('K')
    secondary_min = Q_(loop_res["Secondary Minimum Temperature (C)"],
                       'degC').m_as('K')
    secondary_P_high = Q_(loop_res["Secondary Pressure (kPa)"],
                          'kPa').m_as('Pa')
