...
def compute_Nu_Chen(Re,
    ...
    htc = S*0.00122*(k_f**0.79*cp_f**0.45*rho_f**0.49*
                     (sigma**0.5*mu_f**0.29*h_fg**0.24*rho_g**0.24)**-1)*(
...
def compute_Nu_Groeneveld(Re_g, Pr_g, rho_g, rho_f, x):
    """ Computes the nusselt number in film boiling conditions according to the Groeneveld
        correlation.
    """
    Y = (1 - 0.1*((rho_f/rho_g - 1)**0.4)*((1 - x)**0.4))*(x + rho_f/rho_g*
                                                           (1-x))**-1.06
    return (0.052*(Re_g*(x + (1-x)*rho_g/rho_f))**0.688*(Pr_g**1.26)*Y)
...
def compute_k_SS(T, mat):
    if mat == "SS304":
        return compute_k_SS304(T)
    elif mat == "SS316":
        return compute_k_SS316(T)
    else:
...
def compute_two_phase_friction_multiplier(x,
                                          G,
                                          D_h,
                                          sigma,
                                          rho_l,
